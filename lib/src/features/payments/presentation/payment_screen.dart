...
import 'package:rightlogistics/src/features/orders/data/order_repository.dart';
...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      // 1. MOCK Provider Integration
      await Future.delayed(const Duration(seconds: 3));

      // 2. RECORD Transaction & Order
      ...
      final order = Order(
        ...
        title: widget.post.title ?? 'Fulfillment Purchase',
        quantity: widget.quantity,
        priceAtPurchase: widget.post.price,
        totalAmount: _total,
        serviceFee: _serviceFee,
        tax: _tax,
        currency: widget.post.currency,
        promoCode: _appliedCoupon?.promoCode,
      ...
      await orderRepo.recordTransaction(transaction);

      // 3. FULFILLMENT Logic (Atomic)
      await orderRepo.fulfillPreOrder(
        postId: widget.post.id,
        userId: user.id,
        quantity: widget.quantity,
      );
      ...
      await notificationRepo.sendNotification(
        NotificationModel(
          ...
          title: 'New Order: ${widget.post.title}',
          message:
              '${user.name} purchased ${widget.quantity} units for ${widget.post.currency} ${_total.toStringAsFixed(2)}.',
          timestamp: DateTime.now(),
          type: NotificationType.shipment,
          isRead: false,
          senderId: user.id,
          senderName: user.name,
          targetUserId: widget.post.vendorId,
        ),
      );
